...
+ [新增] 在线同款视频模板平台支持作者发布固定版本、受邀用户填写素材、自配方舟渠道、隔离任务与下载；服务部署和真实生成待验收。

+ [修复] 方舟首帧与首尾帧视频使用自适应比例，避免自动比例被转换为 16:9 后遭接口拒绝。

+ [新增] Seedance 原声可在本机识别逐句字幕，校对文字与时间后烧录合成；修改字幕保留历史成片且不重新生成视频。

+ [新增] Seedance 完整视频工作流由本地 Agent 保存任务快照、查询和下载原片，支持失败续跑、接回已有任务、字幕与 Logo 拼接，并将原片和成片放回画布。

+ [新增] 新增本地版本化视频工作流模板，可从画布发布、暴露文字与图片变量，并以同款复刻或相似创作模式创建独立快照后顺序生成。
+ [新增] 模型渠道新增火山方舟协议，Seedance 视频任务支持提交、任务 ID 持久化、刷新续查与结果下载。
