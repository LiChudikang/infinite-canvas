...
本分支在原项目基础上探索视频模板工作流：作者搭好分镜、提示词和生成参数，使用者只填写文字、上传素材，即可制作自己的版本。保留原作者信息；上游介绍与社区入口见下文。

目前已增加独立在线模板平台入口 `/studio`：作者发布多种固定版本模板，使用者填写素材与文字，配置自己的方舟渠道并查看生成任务。服务端保存模板、隔离用户任务并加密渠道凭证；**尚未部署或完成真实生成与浏览器验收**。启动配置见 [在线模板平台说明](docs/template-platform.md)。

本地画布与本地模板库仍保存在浏览器中，不会随 clone 自动共享；作者需要在在线平台逐个上架。Git 分享的是代码，固定版本链接用于已配置运行的平台。

详细使用步骤、固定范围、实施阶段和验收标准见 **[固定画布共享与未来规划](README.roadmap.md)**。本仓库代码地址为 [LiChudikang/infinite-canvas](https://github.com/LiChudikang/infinite-canvas)；下方原项目安装示例使用上游地址，体验本分支时请替换为此仓库。
