...
目前已有浏览器本地模板库、版本快照、文字/图片变量及本地 Seedance 工作流代码，仍有待验收项。**Git 仓库分享的是代码；你的画布和模板保存在浏览器中，不会随 clone 自动交给别人。** 当前模板页面链接也不能跨用户直接分享。

现在可用画布导出/导入交接一个可编辑副本；未来规划是“发布固定版本 → 分享链接 → 填写变量 → 生成 → 下载”，由服务端约束固定参数并保存任务。
