...
- 本项目的目标不是单纯的 AI 视频画布，而是“视频模板工作流平台”：模板作者搭好流程并锁定关键参数，普通用户只替换少量素材和文字，即可复刻同款或生成相似版本。
- 产品中心是“发布模板 → 填写变量/上传素材 → 复制版本快照 → 异步生成 → 查看进度 → 下载或再做一个”，不要把工作重心偏移到通用画布功能。
- 模板必须区分“同款复刻”和“相似创作”：同款复刻锁定分镜、运镜、时长和提示词主体，只替换允许的主体素材；相似创作保留风格和故事结构，可改写文案、镜头内容及明确开放的局部运镜。
- 模板作者可锁定模型、时长、比例、分辨率、运镜、风格、镜头数量、提示词结构、首尾帧关系和后处理参数；普通用户只能修改模板明确暴露的人物、产品、场景、主题、台词、品牌名及允许的横竖版变量。
- 每次运行必须生成独立模板快照并记录 `templateId`、`version`、变量值、锁定字段、节点、连线、Seedance 配置和后处理配置；已发布版本应保持不可变，模板升级不得影响历史任务。
- 第一版优先跑通管理员搭建、暴露变量、发布模板、用户填写、调用次数/费用提示、Seedance 生成与轮询、失败续跑、下载和“再做一个”。多人协作、复杂权限、完整剪辑器和公开模板市场不是当前优先事项。

### 当前实现边界

- 当前已实现浏览器本地的版本化工作流模板、文字/图片变量、同款复刻/相似创作、模板快照、模板库、发布入口和“再做一个”。模板与素材仍以浏览器本地存储为主，不得描述为云端公开模板平台。
- 本地 Seedance 工作流按画布横向顺序保存独立快照，由 Agent 提交、查询、下载和合成，支持接回已有任务、失败单段重做、字幕与片尾 Logo，并将原片/成片放回画布；依赖生图、首尾帧接力和独立音乐轨道继续保留在 TODO。
- 当前已增加火山方舟 Ark/Seedance 视频任务的创建、任务 ID 持久化、刷新续查和结果下载，但尚未完成构建、完整 UI 验收或真实付费请求验证，不得声称已经生产可用。
- Ark 默认 API 根地址为 `https://ark.cn-beijing.volces.com/api/v3`，视频任务使用 `POST /contents/generations/tasks` 创建并通过 `GET /contents/generations/tasks/{task_id}` 查询；模型参数和能力范围应以用户当前账号及官方文档为准。
- 浏览器直连 Ark 遇到 CORS 时应引导使用现有本地代理，不要把 API Key 写入源码、日志、文档、截图或 Git；任何费用估算在没有可靠单价时只展示调用次数，不得虚构价格。

### 视频内容原则

- 视频模型主要用于人物、酒店空间、产品氛围和单一动作镜头；真实产品界面、中文、人名、价格、经营指标和 Logo 应使用真实素材后期合成。
- 每个生成镜头只承载一个核心动作，长片优先拆段生成并用首尾帧或参考素材保持连续性，再进行剪辑拼接。
- 不得让浮空 UI、乱码、错误指标或虚构功能进入正式产品宣传片；对外发布前必须逐帧检查关键文字、设备边框、人物一致性、黑帧、画面漂移和音画效果。
...
- “完整视频工作流”的方舟密钥只由本地 Agent 从 ARK_API_KEY 或 ARK_API_KEY_FILE 读取，勿与旧的浏览器直连渠道混淆。提交结果不明时必须保留标记和快照，不自动重新创建付费任务。
